...
def likes(request, article_pk):
    if request.user.is_authenticated:
        ...
        if article.like_users.filter(pk=request.user.pk).exists():      # 메모리를 줄일 수 있다
        # if request.user in article.like_users.all():        # 이미 누른 적이 있다면 취소 
            # 좋아요 취소
            article.like_users.remove(request.user)
            liked = False       # 자바스크립트땜에 넣어줌
        else:
            # 좋아요 누름
        ...
        # return redirect('articles:index')
        return JsonResponse(like_status)            # Json 으로 넘겨주는 것
